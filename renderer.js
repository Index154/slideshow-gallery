...
let fallbackImage = 'file://' + path.join(__dirname, 'icons', 'no-images.png')
...
// Image amount / count setting
let imgDim = 375
let imgCount = 8
if(config.imageCount == 'one') {
	imgDim = 700
	imgCount = 1
}

// Get settings from config and update the UI
...
for(i = 0; i < imgCount; i++){
	...
	startingImages[i] = '<img id="img' + i + '" src="file://' + img + '" width="' + imgDim + '" height="' + imgDim + '" style="border-style: solid; border-width: 3px; border-color: white"></img>'
}
